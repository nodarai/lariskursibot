...
from models import Rate, RefCurrency, initialize_db
...
class currency_reader(ContextDecorator):
    ...
    def read_meta(self):
        ...
        for c in currencies:
            ...
            if not currency_exists:
                rc = RefCurrency(
                    name=names[c][0], quantity=quantities[c][0], code=codes[c][0]
                )
    ...
    def read_data(self):
        ...
        eur_id = (
            self.db_session.query(RefCurrency)
            .filter(RefCurrency.code == "EUR")
            .one()
            .id
        )
        usd_id = (
            self.db_session.query(RefCurrency)
            .filter(RefCurrency.code == "USD")
            .one()
            .id
        )
...
if __name__ == "__main__":
    ...
    logging.debug(
        "Got %s as data file and %s as config file", args.data_file, args.config
    )
